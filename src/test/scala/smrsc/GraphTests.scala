...
class GraphTests extends AnyFunSuite:
  ...
  val g1: IGraph =
    Forth(1, List(Back(1), Forth(2, List(Back(1), Back(2)))))

  val g_bad_forth: IGraph =
    Forth(1, List(Back(1), Forth(-2, List(Back(3), Back(4)))))
  ...
  val l2: ILazyGraph =
    Build(
      1,
      List(
        List(Build(2, List(List(Stop(1), Stop(2))))),
        List(Build(3, List(List(Stop(3), Stop(1)))))
      )
    )
  ...
  val l_empty: ILazyGraph =
    Build(1, List(List(Stop(2)), List(Build(3, List(List(Stop(4), Empty))))))
  ...
  val l_bad_forth: ILazyGraph =
    Build(1, List(List(Stop(1), Build(-2, List(List(Stop(3), Stop(4)))))))
  ...
  test(testName = "lazy bad forth cl") {
    assert(
      cl_empty_and_bad(ibad)(l_bad_forth) ==
        Empty
    )
  }
  ...
  test(testName = "min size cl") {
    assert(
      cl_min_size(l3) ==
        Build(1, List(List(Build(3, List(List(Stop(4)))))))
    )
  }
  ...
  test(testName = "min size cl unroll") {
    ...
    assert(
      min_g ==
        Forth(1, List(Forth(3, List(Back(4)))))
    )
  }
