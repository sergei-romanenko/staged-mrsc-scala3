...
object Futurebus extends CountersWorld:
  ...
  val rules: List[Rule] = List(
    ...
    { // r5
      case List(i, sU, eU, eM, pR, pW, pEMR, pEMW, pSU)
          if pR >= 2 && pSU === 0 && pEMR === 0 =>
        List(i, sU + pR, eU, eM, 0, pW, 0, pEMW, 0)
    },
    { // r6
      case List(i, sU, eU, eM, pR, pW, pEMR, pEMW, pSU)
          if pR === 1 && pSU === 0 && pEMR === 0 =>
  ...
  val isUnsafe: C => Boolean =
    ...
    case List(i, sU, eU, eM, pR, pW, pEMR, pEMW, pSU) if pR >= 1 && pW >= 1 =>
      true
...
object Xerox extends CountersWorld:
  ...
  val rules: List[Rule] = List(
    { // (1) rm1
      case List(i, sc, sd, d, e)
          if i >= 1 && d === 0 && sc === 0 && sd === 0 && e === 0 =>
        List(i - 1, 0, 0, 0, 1)
    },
    { // (2) rm2
